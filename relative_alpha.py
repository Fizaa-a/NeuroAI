...
def calculate_relative_power(theta_power, alpha_power, beta_power, gamma_power):
    ...
    # Calculate relative power of the alpha wave
    rp_alpha = alpha_power / total_power
...
def main():
    ...
    for file_path in file_paths:
        if file_path.endswith('_1.edf'):
            try:
                ...
                # Process EEG data
                rp_alpha_list = []  # List to store rp_alpha values for dataset
                for electrode in range(num_electrodes):
                    ...
                    rp_alpha = calculate_relative_power(absolute_power_theta, absolute_power_alpha, absolute_power_beta,
                                                        absolute_power_gamma)
                    rp_alpha_list.append(rp_alpha)

                # Construct row data
                row_data = {'Patient_ID': file_name, **{channel_name: val for channel_name, val in zip(channel_names, rp_alpha_list)}}
    ...
    try:
        results_df_1.to_csv('Features/relaxed_alpha_power.csv', index=False)  # For files ending with '_1.edf'
        print("CSV file for _1.edf files created successfully.")
    except Exception as e:
        print(f"Error writing CSV file for _1.edf files: {str(e)}")

    try:
        results_df_2.to_csv('Features/stressed_alpha_power.csv', index=False)  # For files ending with '_2.edf'
